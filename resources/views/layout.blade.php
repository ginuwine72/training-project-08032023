...
<head>
    ...
    <link href="{{ asset('css/te.css') }}" rel="stylesheet">
</head>
<body>
<div class="container">
    <div class="row">
                    ...
                    @if(session('cart'))
                        @foreach(session('cart') as $id => $products)
                            <div class="row cart-detail">
                                ...
                                <img src="{{ asset('product/'.$products['image']) }}">
                           </div>
                                <div class="col-lg-8 col-sm-8 col-8 cart-detail-product">
                                    <p></p>
                                    <span class="price text-info"> ${{ $products['price'] }}</span> <span class="count"> Quantity:{{ $products['quantity'] }}</span>
                                </div>
                            </div>
                        @endforeach
                    @endif
